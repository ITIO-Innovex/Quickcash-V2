...
function WidgetsValueModal(props) {
  ...
  const widgetTypeTranslation = (`widgets-name.${currWidgetsDetails?.type}`);
  ...
  const getWidgetType = (type) => {
    switch (type) {
            ...
            {isSignTypes ? (
              ...
              <div>
                ...
                <div className="mx-3 mb-6 mt-3">
                  <p>{("At least one signature type should be enabled.")}</p>
                </div>
              </div>
            )}
          </div>
      ...
      case textInputWidget:
        return (
          <input
            placeholder={hint || ("widgets-name.text")}
  ...
  const HandleRequiredField = () => {
    ...
    return (
      <span className="text-center text-[12px]">
        {("required-mssg", { leftRequiredWidget, totalWidget })}
      </span>
    );
  };
          ...
          {isFinish ? (
            <>
              <div className="p-1 mt-3">
                <span className="text-base">{("finish-mssg")}</span>
              </div>
