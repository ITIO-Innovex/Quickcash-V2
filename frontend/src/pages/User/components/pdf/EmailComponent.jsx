...
function EmailComponent({
  ...
  const sendEmail = async () => {
    setIsLoading(true);
    const params = { docId: pdfDetails?.[0]?._id, recipients: emailList };
    // const sendmail = await Parse.Cloud.run("forwarddoc", params);
    const { data: sendmail } = await axios.post(API_ROUTES.FORWARD_DOC, params, { headers: { Authorization: getBearerToken() } });
        ...
        <ModalUi isOpen showHeader={false}>
          ...
          <div className="h-full p-[20px]">
            ...
            {emailList.length > 0 ? (
              ...
              <div>
                <input
                  ...
                  onChange={handleEmailValue}
                  onKeyDown={handleEnterPress}
                  placeholder={("enter-email-plaholder")}
                  onBlur={() => emailValue && handleEnterPress("add")}
                  onInvalid={(e) =>
                    e.target.setCustomValidity(("input-required"))
                  }
            ...
            <div className="bg-[#e3e2e1] mt-[10px] p-[5px] rounded">
              <span className="font-bold">{("report-heading.Note")}: </span>
              <span className="text-[15px]">{("email-error-2")}</span>
            </div>
