...
import { CONTACT_ROLES } from "../../constant/const.js";
...
function WidgetComponent(props) {
  ...
  // Helper to check if current selected recipient is a viewer
  const isCurrentViewer =
    props?.signersdata?.[props?.isSelectListId]?.UserRole?.toLowerCase() === 'viewer';

  return (
    <>
      {isMobile ? (
        !props.isMailSend && !isCurrentViewer && (
        ...
        >
          {
            props?.isSelectListId?.toString()
            && !isCurrentViewer
            && (
              <>
                <div className="mx-2 pr-2 pt-2 pb-1 text-[15px] text-base-content font-semibold border-b-[1px] border-base-300">
                  <span>{("Drag-and-Drop Signature Components")}</span>
                </div>
      ...
      {isSignersModal && (
        <ModalUi
          title={props.title ? props.title : ("recipients")}
        ...
        >
          {props.signersdata.length > 0 ? (
            ...
            <div className=" p-[20px] text-[15px] font-medium text-center">
              {("please-add")} {props.title ? props.title : ("recipients")}
            </div>
          )}
        </ModalUi>
      )}
