...
import { DOCUMENT_STATUS, themeColor } from "./const";
...
export const contractUsers = async () => {
  try {
    const { data: userDetails } = await axios.get(API_ROUTES.GET_USER_DETAILS, {
      headers: { Authorization: getBearerToken() }
    });
    let data = [];
    if (userDetails?.data) {
      const json = JSON.parse(JSON.stringify(userDetails.data));
      data.push(json);
    }
    return data;
  } catch (err) {
...
export const multiSignEmbed = async (widgets, pdfDoc, signyourself, scale) => {
  ...
  if (!hasError) {
    ...
    return {
      error:
        "This pdf is not compatible with opensign please contact <info@itio.in>"
    };
  }
};
...
export const contractDocument = async (documentId) => {
  ...
  const response = await axios.get(API_ROUTES.GET_DOCUMENT(documentId), {
    headers: { Authorization: getBearerToken() }
  });
...
export const getTenantDetails = async (objectId, contactId) => {
  try {
    ...
    const res = await axios.get(API_ROUTES.GET_CONTACT(contactId));
    if (res.data.result) {
  ...
  try {
    ...
    iframe.onload = () => {
      try {
        ...
        document.body.removeChild(iframe);
        setIsDownloading("");
        alert("Unable to print. Please try downloading the PDF first.");
      }
    };
    ...
    iframe.onerror = () => {
      ...
      setIsDownloading("");
      alert(
        "Unable to load PDF for printing. Please try downloading the PDF first."
      );
    };
  } catch (err) {
    console.error("Print error:", err);
...
export const convertBase64ToFile = async (pdfName, pdfBase64) => {
  ...
  try {
    ...
    const { data: fileRes } = await axios.post(API_ROUTES.SAVE_FILE, body, {
      headers: { Authorization: getBearerToken() },
    });
...
export const mailTemplate = (param) => {
  const appName = "Quick Cash";
  const logo = `<div style='padding:10px'><img src='https://itio.in/images/829775694_ITIO%20LOGO%20_general%20ICON-01-min.PNG' height='50' /></div>`;
...
// Add these new functions for token refresh handling
...
export const signPdfFun = async (
  ...
  try {
    ...
    const response = await axios.post(API_ROUTES.SAVE_PDF, params);
...
export const getDocumentStatus = (document) => {
  ...
  if (!document?.IsCompleted && !document?.SentToOthers) {
    return DOCUMENT_STATUS.draft;
  }

  if (!document?.IsCompleted && document?.SentToOthers) {
    return DOCUMENT_STATUS.inProgress;
  }
}

export const getDocumentStatusChip = (document) => {
