...
function PdfRequestFiles(
  ...
  const [viewerSigners, setViewerSigner] = useState([]);
  ...
  if (getQuery) {
    sendmail = getQuery?.[2]?.split("=")[2]; //false
  }
  ...
  const getDocumentDetails = async (
    ...
    try {
      ...
      const senderUser = localStorage.getItem('UserInformation');
      ...
      if (documentData && documentData.length > 0) {
        ...
        const getCurrentSigner = getSigners?.findIndex(
          (data) => data.UserId === jsonSender?.objectId
        );
        ...
        markDocumentView(currUserId, docId);
        if (currUserId) {
        ...
        if (isCompleted) {
          ...
          if (!isSuccessPage) {
            setIsCompleted({
              isModal: true,
              message: ("document-signed-alert-1")
            });
          }
        } else {
          if (currUserId) {
            ...
            if (checkCurrentUser?.placeHolder) {
              const widgetId = [];
        ...
        placeholdersOrSigners = placeholdersOrSigners.filter((unfiltered) => unfiltered.UserRole !== CONTACT_ROLES.Viewer);
        ...
        if (documentData?.[0]?.Signers) {
          setViewerSigner(documentData.at(0).Signers.filter((viewerSigner) => viewerSigner.UserRole === CONTACT_ROLES.Viewer))
        }
          ...
          if (checkboxExist && requiredCheckbox && showAlert) {
            ...
            if (documentData && documentData.length > 0) {
              ...
              if (arrayBuffer === "Error") {
                ...
                setIsAlert({
                  isShow: true,
                  alertMessage: "The file is not a valid PDF or could not be loaded."
                });
                return;
              } else {
            ...
            try {
              ...
              if (!pdfBytes?.error) {
                const objectId = pdfDetails?.[0]?.ExtUserPtr?._id;
                ...
                if (resSign && resSign.status === "success") {
                  ...
                  const index = pdfDetails?.[0]?.Signers.findIndex(
                    (x) => x.objectId === signerObjectId && x.UserRole !== CONTACT_ROLES.Viewer
                  );
                  const newIndex = pdfDetails?.[0]?.Signers.findIndex(
                    (x) => x.objectId !== signerObjectId && x.UserRole !== CONTACT_ROLES.Viewer
                  );
                  const usermail = {
                  ...
                  if (sendmail !== "false" && sendInOrder) {
                    ...
                    if (user) {
                      ...
                      try {
                        ...
                        let signPdf =
                          `${hostUrl}/digital-signature/login/${encodeBase64}`;
                  ...
                  if (!isSuccessRoute) {
                    ...
                    window.location.href = `/digital-signature/success?${params}`;
                  }
                } else {
                  setIsUiLoading(false);
                  setIsAlert({
              ...
              if (err && err.message.includes("is encrypted.")) {
                setIsAlert({
                  isShow: true,
                  alertMessage: ("encrypted-pdf-not-support")
                });
              } else {
  ...
  const tourConfig = [
    {
      ...
      content: minRequiredCount
        ? ("Please confirm that you have selected at least {{minRequiredCount}} checkboxes.", { minRequiredCount })
  ...
  const declineDoc = async (reason) => {
    ...
    await axios
      .put(API_ROUTES.DECLINE_DOCUMENT, params, {})
      .then(async (result) => {
  ...
  const formatArrayToString = (arr) => {
    ...
    return `${arr.join(", ")} ${("and")} ${lastElement}`; // Format the string
  };
  const requestSignTourFunction = () => {
    ...
    const tourConfig = [
      {
        ...
        content: () => (
          <TourContentWithBtn
            message={("tour-mssg.pdf-request-file-6", { pagenumbers })}
      ...
      },
      {
        ...
        content: () => (
          <TourContentWithBtn
            message={("tour-mssg.pdf-request-file-1")}
            isChecked={handleDontShow}
          />
        ),
        position: "top",
      ...
      },
      {
        ...
        content: () => (
          <TourContentWithBtn
            message={("tour-mssg.pdf-request-file-2")}
            isChecked={handleDontShow}
          />
        ),
        position: "top",
      ...
      },
      {
        ...
        content: () => (
          <TourContentWithBtn
            message={("tour-mssg.pdf-request-file-3")}
            isChecked={handleDontShow}
          />
        ),
        position: "top",
    ...
    const signedByStep = {
      ...
      content: () => (
        <TourContentWithBtn
          message={("tour-mssg.pdf-request-file-4")}
          isChecked={handleDontShow}
        />
      ),
      position: "top",
    ...
    const defaultSignStep = {
      ...
      content: () => (
        <TourContentWithBtn
          message={("tour-mssg.pdf-request-file-5")}
          isChecked={handleDontShow}
        />
      ),
      position: "top",
    ...
    const allModifyWidgets = {
      ...
      content: () => (
        <TourContentWithBtn
          message={("tour-mssg.allowModify-widgets")}
          isChecked={handleDontShow}
        />
      ),
      position: "top",
  ...
  const handleDeclineMssg = () => {
    ...
    return (
      <div>
        ...
        <div className="mt-2">
          {" "}
          <span className="font-medium">{("decline-by")}</span> : {user}
        </div>
        <div className="mt-2">
          {" "}
  ...
  const AgreementTour = [
    {
      ...
      content: () => <p className="p-0">{("agrrement-alert")}</p>,
      position: "top",
      style: { fontSize: "13px" }
    }
  ];
  ...
  const markDocumentView = async (userId, docId) => {
    try {
      const payload = {
        userId,
        documentId: docId
      };
      const response = await axios.put(API_ROUTES.UPDATE_DOCUMENT_VIEWED, payload);
    } catch (error) {
      console.log('error', error);
    }
  }
  ...
  return (
    <DndProvider backend={HTML5Backend}>
        ...
        <div>
          ...
          >
            ...
            <PdfDeclineModal
              show={isExpired}
              doc={pdfDetails?.[0]}
              headMsg={("expired-doc-title")}
              bodyMssg={("expired-on-mssg", { expiredDate })}
              isDownloadBtn={true}
                  ...
                  <div className="h-full p-[20px] text-base-content">
                    {isCompleted?.message ? (
                      ...
                      <div className="px-[15px]">
                        <span>{("Congratulations! 🎉 This document has been successfully signed by all participants!")}</span>
                      </div>
                    )}
                    {!isCompleted?.message && (
                      <div className="flex flex-col mt-3 gap-1 px-[10px] justify-center items-center">
                        ...
                        <div className={`${!redirectUrl ? "m-2" : ""}`}>
                          ...
                          >
                            <MilitaryTechIcon sx={{ fontSize: "16px" }} />
                            <span className="hidden lg:block">
                              {("Certificate")}
                            </span>
                          </button>
      ...
      <ModalUi
        ...
        title={isAlert?.title || ("Alert message")}
