...
function PlaceHolderSign() {
    ...
    if (documentData && documentData.length > 0) {
      ...
      if (alreadyPlaceholder) {
        ...
        if (isCompleted) {
          ...
          setIsAlreadyPlace({
            status: true,
            message: ("document-signed-alert-6")
          });
        } else if (currDate > expireUpdateDate) {
          // If document has expired
          setIsAlreadyPlace({
            status: true,
            message: ("document-signed-alert-7")
          });
        } else {
      ...
      //this case happen using placeholder form and load first time
        ...
        const currEmail = documentData[0].CreatedBy.email;
  ...
  const alertSendEmail = async () => {
    // Only require signature fields for non-viewer roles
    const filterPrefill = signerPos?.filter((data) => {
      const signerDetails = signersdata?.find((item) => item._id === data?.signerObjId);
      // Skip signature requirement for viewers
      return data.Role !== "prefill" && signerDetails?.UserRole?.toLowerCase() !== CONTACT_ROLES.Viewer;
    });
  ...
  const autosavedetails = async () => {
    ...
    try {
      ...
      if (signerPos?.length > 0) {
        payload.Placeholders = signerPos;
      }
  ...
  const saveDocumentDetails = async () => {
    ...
    if (pdfUrl) {
      ...
      if (
        ...
        const currentUserMail = JSON.parse(localStorage.getItem('UserInformation'))?.email;
  ...
  const sendEmailToSigners = async () => {
    ...
    console.log('signerMail', signerMail)
    ...
    for (let i = 0; i < signerMail.length; i++) {
      try {
        ...
        sendMail = await axios.post(API_ROUTES.SEND_MAIL, params, { headers: headers });
      } catch (error) {
  ...
  const tourConfig = [
    ...
    },
    {
      ...
      content: () => (
        <TourContentWithBtn
          message={("Click this button to add more recipients/signers to the document.")}
          isChecked={handleDontShow}
        />
      ),
      position: "top",
    ...
    },
    {
      ...
      content: () => (
        <TourContentWithBtn
          message={("Clicking 'Next' will save the document. In the next step you can customize the emails to be sent out to the recipients or copy the signing links and share those with the recipients yourself.")}
          isChecked={handleDontShow}
        />
      ),
      position: "top",
  ...
  const handleRecipientSign = () => {
    if (currentId) {
      navigate(`/digital-signature/recipientSignPdf/${documentId}/${currentId}`);
    } else {
      navigate(`/digital-signature/recipientSignPdf/${documentId}`);
    }
  };
    ...
    if (isAddSigner) {
      ...
      if (data && data.objectId) {
        ...
        const updateSigner = signersdata.map((x) => {
          if (x.Id === id) {
            // Set UserRole dynamically if present
            return { ...x, ...data, className: "contracts_Contactbook", UserRole: data.UserRole || x.UserRole };
          }
          return { ...x };
        });
        if (updateSigner && updateSigner.length > 0) {
          const currEmail = pdfDetails[0].CreatedBy.email;
  ...
  const signatureWidgetTour = [
    {
      ...
      content: ("A minimum of one signature field must be added for each user. You have not added signature field."),
      position: "top",
      style: { fontSize: "13px" }
    }
  ];
  ...
  const handleRoleSelect = (role, _, documentId) => {
    try {
      axios.patch(API_ROUTES.UPDATE_USER_ROLES, { id: documentId, role }, { headers: { Authorization: getBearerToken() } });

      setSignersData((prev) => {
        const copy = [...prev];

        const indexToUpdate = copy?.findIndex((item) => item?._id === documentId);
        if (indexToUpdate !== -1) {
          copy[indexToUpdate].UserRole = role;
        }
              ...
              <div className=" w-full md:w-[57%] flex mr-4">
                ...
                <div className=" w-full md:w-[95%] ">
                  ...
                  <ModalUi
                    ...
                    title={isSendAlert.mssg === "confirm" && ("Send Mail")}
                  ...
                  >
                    <div className="max-h-96 overflow-y-scroll scroll-hide p-[20px] text-base-content">
                      {isSendAlert.mssg === "confirm" && (
                        <>
                          {!isCustomize && (
                            <span>{("Are you sure you want to send out this document for signatures?")}</span>
                          )}
                          {
                            isCustomize && (
                              <>
                      ...
                      {mailStatus === "success" ? (
                        <div className="text-center mb-[10px]">
                          ...
                          {pdfDetails[0].SendinOrder ? (
                            <p>
                              {isCurrUser
                                ? ("Subsequent signers will get email once you signs the document.")
                                : (`You have successfully sent email to ${signersdata[0]?.Name}.`)}
                            </p>
                          ) : (
                            <p>{("You have successfully sent email to ektabhardwaj. Subsequent signers will get email(s) once ektabhardwaj signs the document")}</p>
                          )}
                          {isCurrUser && <p>{("Do you want to sign the document right now?")}</p>}
                        </div>
                      ) : mailStatus === "quotareached" ? (
                        ...
                        <div className="mb-[10px]">
                          {mailStatus === "dailyquotareached" ? (
                            ...
                            <p>{("placeholder-alert-6")}</p>
                          )}
                          {isCurrUser && (
                            <p className="mt-1">{("Do you want to sign the document right now?")}</p>
                          )}
                        </div>
                      )}
                      {!mailStatus && (
                        ...
                        >
                          ...
                          <button
                            onClick={() => {
                              ...
                              navigate("/digital-signature/analytic-data");
                            }}
                            type="button"
